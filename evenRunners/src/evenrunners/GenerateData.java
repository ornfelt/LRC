...
/**
 * Program for generating data used for further demonstrating my proof
 *
...
public class GenerateData {
    ...
    IO io = new IO();
    String fp = "GenerateData.txt";
    String fpTwo = "GenerateDataPeriodList.txt";
    String input;
    int evenInput;
    String[] arrSplit;
...
//        Constant needed to adjust velocities in the animation. Calculated with brute force (may need to be adjusted, as it differs depending on the computer running the code.
    ...
    public void kRunners(int currentRunner, boolean firstRun, int preK, double[] preV) throws IOException, InterruptedException {
        ...
        if (firstRun) {
            ...
            String choiceText = "50";

            while (!inputDone) {
                input = showInputDialog(null, "Enter how much data you want to generate for k: " + k, choiceText);
                evenInput = showConfirmDialog(null, "Do you want to generate even values for all Δv?", "Select an option...", YES_NO_OPTION);
            ...
            int vAmount = Integer.parseInt(input);
            double[] vInput = new double[vAmount / evenInput];
            for (int i = 0; i < vInput.length; i++) {
                vInput[i] = vAmount - (evenInput * i);
            }
            v = vInput;

//            0 = yes, 1 = no, 2 = cancel
            int fileOption = showConfirmDialog(null, "Do you wish to delete old textfiles?", "Select an option...", YES_NO_OPTION);
