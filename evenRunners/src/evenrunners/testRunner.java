...
/**
 * "Cheat-program" that allows testing with k+1 runners, instead of k
 *
...
public class testRunner {
    ...
    IO io = new IO();
    String fp = "testRunnerList.txt";
    String fpTwo = "testRunnerPeriodList.txt";
    String fpThree = "testRunnerAnimationList.txt";
    String input;
    String[] arrSplit;
...
//        Constant needed to adjust velocities in the animation. Calculated with brute force (may need to be adjusted, as it differs depending on the computer running the code.
    ...
    public void kRunners(int currentRunner, boolean firstRun, int preK, double[] preV) throws IOException, InterruptedException {
        ...
        if (firstRun) {
            ...
            /**
             * Saves all velocities in double array For this class testRunner, k+1 velocities are being saved (1-k+1). This is done to demonstrate that it's the first
             * runner that's being checked (the one never being lonely).
             */
            double[] vInput = new double[k + 1];
...
//            0 = yes, 1 = no, 2 = cancel
            int fileOption = showConfirmDialog(null, "Do you wish to delete textfiles?", "Select an option...", YES_NO_OPTION);
...
//    length = arr.length, r = the velocity that dV will be calculated from
    private double[] dVelocity(double[] n, int length, int r) {
        ...
        for (int i = 0; i < n.length - 1; i++) {
            ...
            dv[i] = Math.abs(dv[i]);
//            System.out.println("dv equals: " + dv[i]);
        }
...
//            System.out.println("dvRatio equals: " + dvRatio[i]);
        }
        return dvRatio;
    }
...
//                System.out.println("tempoInt equals: " + tempoInt);
                    if ((result[r] - tempoInt) < kth || (result[r] - tempoInt) > (kth * (k - 1))) {
                        notLonely.add(p);
...
//    Function that starts animation
    public void runnerAnimation(double[] aV, ArrayList aniR, int btnValue, boolean notLonelyAni) throws InterruptedException {
        Circle.startAnimation(aV, aniR, btnValue, notLonelyAni);
    }
}
